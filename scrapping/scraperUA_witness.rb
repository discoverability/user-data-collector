...
browser.text_field(id: "add-profile-name").set "#{$witness_username}"
...
browser.span(text: "#{$witness_username}").click
...
File.write("results_#{time}_#{$witness_username}_snap1.csv", thumbs_final)
...
scheduler.at "#{$day2}" do
    ...
    while File.exist?("results_#{time}_#{$profile2_username}_snap2.csv") == false
        sleep(5)
    end
    ...
    File.write("results_#{time}_#{$witness_username}_snap2.csv", thumbs_final)
end
